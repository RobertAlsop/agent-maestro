...
An agent system includes everything required for agents to operate reliably: orchestration, workflows, authority constraints, and memory. It does not include the organizational strategy that determines *what* to automate (see [[Agent Maestro — Strategy]]) or the infrastructure that hosts execution.

## Purpose

Agent systems are the execution layer of [[Operational Intelligence]]. They translate organizational processes into automated, governed operations. A well-formed agent system makes the boundary between human responsibility and agent responsibility explicit and auditable at every point.
