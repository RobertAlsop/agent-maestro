...
schema_version: 2
title: "Vault Structure and Knowledge Upgrade v2"
status: active
...
superseded_by: []
---
# (This Task has already been completed but the content of this prompt could be generalised and reused.)
We are now going to plan and execute a comprehensive structural and knowledge upgrade of the **AGENT_MAESTRO** Obsidian folder in:
...
## 5. Every note must conform to one of the templates in
...
The vault contains several canonical object classes.  
Each class may have its own templates, maps, and registry behavior.
...
Where relevant, these objects must be resolvable through the appropriate registry.

---

# Index vs Registry distinction

Indexes and Registries serve different roles.

**Indexes**  
Human navigation lists that organize related notes.

**Registries**  
Authoritative system records that define resolvable objects.

Example:

|Object|Type|
|---|---|
|Authority Pack Registry|Registry|
|Authority Packs Index|Index|

Registries resolve canonical objects.  
Indexes assist navigation.

---

# Authority architecture vs pack implementation

The vault separates **authority architecture design** from **pack implementation**.
...
These are the **actual pack instances used by systems and constellations**.

---

# Shared Services vs Infrastructure

These layers have different purposes.

**Shared Services**

Reusable operational capabilities used by agents, such as:
...
Underlying runtime and technical environment, such as:
...
Concepts represent **core ontology elements**.
...
Domains represent **areas of application or study**.
...
Concepts define system elements.  
Domains define fields in which those elements operate.
...
# Critical planning rule (large-vault safety)

During planning, treat the existing vault as the **source of truth**.

Never assume a note is missing until the vault has been scanned for it.

If a concept appears multiple times, prefer **linking to the canonical note** rather than creating duplicates.
