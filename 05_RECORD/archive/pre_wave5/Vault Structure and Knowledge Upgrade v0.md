We are now going to plan and execute a comprehensive structural and knowledge upgrade of the AGENT_MAESTRO vault in:

/Users/robertalsop/obsidian/

Inputs:
- Existing AGENT_MAESTRO vault
- Existing approved master templates
- Governance documents already provided in the project sources
- Any new templates that are created and approved before build
...
- refined maps that connect executive layer, shared services, constellations, inheritance, and compiled governance
...
5. Every note must contain the following sections:
...
Concept notes should include Definition.  
System notes should include Responsibilities.

6. The upgrade must also include two additional canonical architecture components:
...
System Registry must be treated as a canonical vault-wide registry for systems and should track, where relevant:
- system identity
- system type
- lifecycle status
- owner
...
If any required template does not yet exist, stop and output exactly:
...
- minimum frontmatter fields required

Do not continue until templates are supplied and approved.
...
9. Every concept, system, pipeline, compiler artifact family, and pack family must appear in at least one map.
...
21. References, indexes, and maps must be updated so the upgraded architecture remains navigable.
...
Final requirement:
Before making any file changes, Claude must think carefully about canonical note placement, avoid duplication, and preserve a coherent long-term architecture for Agent Maestro.